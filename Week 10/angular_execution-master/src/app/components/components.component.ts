...
@Component({
  ...
  <h1>Components</h1>
    ...
    <a (click)="setValues('Fast', newVehicle)">
      Click to change to Fast {{ newVehicle }}</a
    >
    <hr />
    <a (click)="setValues(newSpeed, 'Rocket')">
      Click to change to {{ newSpeed }} Rocket</a
    >
    <hr />
    <a (click)="vehicle = 'Car'"> Click to change the vehicle to a Car</a>
    <hr />
    <a (click)="vehicle = 'Enhanced ' + vehicle"> Click to Enhance Vehicle</a>
    <hr />
  `,
  styles: [
    `
